...
public class MyInteraction {
    ...
    public void addMessage(UmlMessage umlMessage, Object source, Object target) {
        ...
        if (target instanceof MyLifeLine) {
            if (((MyLifeLine) target).getState() > 0) {
                ((MyLifeLine) target).changeState();
            } else if (umlMessage.getMessageSort() == MessageSort.DELETE_MESSAGE) {
                ((MyLifeLine) target).changeState();
            }
        }
    }
    ...
    public void checkDelete() throws UmlRule007Exception {
        for (String name : name2lifeLine.keySet()) {
            for (MyLifeLine lifeLine : name2lifeLine.get(name)) {
                if (lifeLine.getState() > 1) {
    ...
    public void checkAttribute(Map<String, Object> id2elements) throws UmlRule006Exception {
        for (String name : name2lifeLine.keySet()) {
            for (MyLifeLine lifeLine : name2lifeLine.get(name)) {
                UmlAttribute attribute = (UmlAttribute) id2elements.get(lifeLine.getRepresent());
                if (!umlInteraction.getParentId().equals(attribute.getParentId())) {
